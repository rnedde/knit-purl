...
function setup() {
  ...
  socket.onmessage = (event) => {
    ...
    if (msg.type === 'init') {
      ...
      // Instead of replacing, queue new stitches:
  ...
  // Hook up your submit button listener to send new messages to server
  let submitBtn = select('#submitBtn');
  submitBtn.mousePressed(() => {
    let userInput = select('#userInput').value();
    if (userInput.trim() !== '') {
      socket.send(JSON.stringify({ type: 'newMessage', data: userInput }));
...
// ... your drawKnit and drawPurl functions
